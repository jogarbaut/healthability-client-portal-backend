...
const createNewTask = async (req, res) => {
  const { user, title, body, dueDate } = req.body

  // Confirm data
  if (!user || !title || !body || !dueDate) {
  ...
  const task = await Task.create({ user, title, body, dueDate })
...
const updateTask = async (req, res) => {
  const { id, user, title, body, dueDate, completed } = req.body

  // Confirm data
  if (!id || !user || !title || !body || !dueDate || typeof completed !== "boolean") {
    return res.status(400).json({ message: "All fields are required" })
  }
